...
import { stableHashIgnoringVolatile } from "../src/part/hash.js";
import { extractLocation, convertFileUrlToUrl, generateMermaid } from "../src/part/util.js";
...
describe("stableHashIgnoringVolatile", () => {
  it("should produce the same hash for objects differing only in volatile fields", () => {
    ...
    const obj2 = {
      ...
      process: {
        uptime: 9999,
        memoryUsage: { rss: 9999 },
        cwd: "/different/path",
      },
    };

    expect(stableHashIgnoringVolatile(obj1)).toBe(
      stableHashIgnoringVolatile(obj2),
    );
  });
  ...
  it("should produce different hashes for objects differing in non-volatile fields", () => {
    ...
    expect(stableHashIgnoringVolatile(obj1)).not.toBe(
      stableHashIgnoringVolatile(obj2),
    );
  });
...
describe("convertFileUrlToUrl", () => {
  it("should convert a file path to a GitHub blob URL", () => {
    ...
    expect(result).toBe(
      "https://github.com/owner/repo/blob/abc123/src/index.js#L42",
    );
  });
  ...
  it("should handle file:// URLs", () => {
    ...
    expect(result).toBe(
      "https://github.com/owner/repo/blob/def456/src/index.js#L10",
    );
  });
});
